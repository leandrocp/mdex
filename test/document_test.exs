defmodule MDEx.DocumentTest do
  ...
  describe "traverse and update" do
    test "modify existing nodes" do
      assert MDEx.Document.traverse_and_update(@document, fn
    ...
    test "append child" do
      assert MDEx.Document.traverse_and_update(~M[# Test], fn
  ...
  describe "traverse and update with accumulator" do
    test "modify existing nodes" do
      ...
      assert MDEx.Document.traverse_and_update(document, 0, fn
    ...
    test "control processing continuation" do
      ...
      assert MDEx.Document.traverse_and_update(document, :cont, fn
               node, :halted ->
                 {node, :halted}
  ...
  describe "Enumerable protocol" do
    ...
    test "member?" do
      assert Enum.member?(@document, %MDEx.Code{literal: "elixir", num_backticks: 1})
      assert Enum.member?(@document, %MDEx.Text{literal: "more"})
    end
